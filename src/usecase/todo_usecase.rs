...
use crate::domain::repositories::todo_repository::TodoRepositoriy;
use async_trait::async_trait;
use uuid::Uuid;

#[derive(Clone)]
pub struct TodoUsecase<T: TodoRepositoriy + Clonse> {
    repository: T,
}

impl<T: TodoRepositoriy + Clone> TodoUsecase<T> {
...
#[async_trait]
impl<T: TodoRepositoriy + Send + Sync + Clone> TodoService for TodoUsecase<T> {
    async fn get_all_todos(&self) -> Result<Vec<Todo>, sqlx::Error> {
        self.repository.find_all().await
    }

    async fn get_todo_by_id(&self, id: Uuid) -> Result<Option<Todo>, sqlx::Error> {
        self.repository.find_by_id().await
    }

    async fn create_todos(&self, title: String, description: String) -> Result<Todo, sqlx::Error> {
